...
class mpagina{
    ...
    function mpagin($nreg){
        $this->nrini =0; // inicia siempre 0(posición) debido a vectores (consultas)
        $this->nrfin = $nreg; 
    ...
    function spag($sqlnp, $nreg, $pag, $bo, $arc){
        ...
        //Calcular el número de paginas que se necesitan para incluir en la visualización
        ...
        $mpag = "<form name 'f2i' action='".$arc."'method='get'>";
        if (is_null($npg))
            $npg=1;
        ...
        //Control de form para ir a cualquier pagina
